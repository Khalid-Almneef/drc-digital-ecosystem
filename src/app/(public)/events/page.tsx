...
function MadaratRegistrationModal({ event, onClose }: { event: UpcomingEvent; onClose: () => void }) {
  ...
  async function submit() {
    ...
    const sessionId = String(event.eventId).startsWith("madarat-") ? String(event.eventId).slice("madarat-".length) : String(event.eventId);
    setSubmitting(true);
    setMessage(null);
    try {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg overflow-hidden rounded-2xl border border-border bg-background shadow-xl">
        ...
        <div className="grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-2">
          ...
          <label className="text-xs text-muted">{tr("Department / major (optional)", "القسم / التخصص (اختياري)")}
